...
\frac{\partial}{\partial M} \log \mathcal{N}(y;a,M) = -\frac{1}{2} \left[ M^{-1} - M^{-1}(y-a)(y-a)^TM^{-1} \right]\label{eq:deriv_gausscov}
\end{eqnarray}
...
\begin{eqnarray}
\begin{split}
\log p_m(g,z \mid x) \sim -\frac{1}{2} \left[ \log \det (C_m) + (x-zABg)^T C_m^{-1} (x-zABg) \right] + \\
+ (\alpha_z-1)\log(z) - \frac{z}{\theta_z} + (\alpha_g-1)\sum_{k=1}^K \log(g_k) - \frac{1}{\theta_g} \sum_{k=1}^K g_k
\end{split} \\
C_m = \sigma_x I + z^2 \sigma_v AA^T
\end{eqnarray}
