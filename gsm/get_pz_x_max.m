...
function lPz_x = calc_pz_x(z, x, Sigma0, ACAT, x0,kGam,thGam)
    ...
    %lPz_x = log(gampdf(z,kGam,thGam)) + normpdfln(x, x0, [], Sigma0+z^2 * ACAT);
    %lPz_x = log(gampdf(z,kGam,thGam)) + log(mvnpdf(x', x0', Sigma0+z^2 * ACAT));
    lPz_x = log(gampdf(z,kGam,thGam)) + loggauss(x, x0, Sigma0+z^2 * ACAT);

    lPz_x=-lPz_x;
    
end

function lp = loggauss(x,mu,sigma)
    logdet = stableLogdet(sigma);
    quad = (x-mu)' * (sigma \ (x-mu));
    const = log(2*pi);
    lp = -(const + logdet + quad) / 2;    
end
