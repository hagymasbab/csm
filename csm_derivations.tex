...
\begin{equation}
\begin{split}
z^{2 D_v} \det(A^TA C(z,g)) = \det \left( z^2 A^TA \left[  \frac{\sigma_x}{z^2} (A^TA)^{-1} + C_v  \right] \right) = \\
= \det \left( \left[  \sigma_x I +  z^2 A^TA C_v  \right] \right) = \det \left( \left[  \sigma_x I +  z^2  A^T C_v A \right] \right) 
\end{split}
\end{equation}
