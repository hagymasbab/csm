function gestaltFindK(Dx,filterset,krange,dataset,trainingSize,batchSize,testSize,emSteps,samples,nTrials)
    ...
    test_likelihoods = zeros(nk,nTrials);
    for t = 1 : nTrials
        fprintf('TRIAL %d\n',t);
        for i = 1:nk
            act_k = krange(i);
            ...
            ge.cc = cc_saved;
            test_likelihoods(i,t) = gestaltLogLikelihood(ge,100,X_test,'scientific',sciLike);
            save('findk_likes.mat','test_likelihoods','krange','ge');
            fprintf('\b test set log-likelihood: %.4f\n',test_likelihoods(i,t));
        end        
    end
end