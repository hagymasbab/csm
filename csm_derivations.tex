...
\end{eqnarray}
%
where K is the fixed number of possible gestalts in the visual scene and $g_k$ is the strength of the gestalt number $k$, coming from a $K$-dimensional Gamma prior distribution with shape and scale  parameters $\alpha_g$ and $\zeta_g$ controlling the sparsity of the prior.

\begin{equation}
p(g) = \textrm{Gam}(g; \alpha_g,\zeta_g)
\end{equation}
%
The global contrast of the image patch is encoded by a scalar variable $z$, also coming from a Gamma prior

\begin{equation}
p(z) = \textrm{Gam}(z; \alpha_z,\zeta_z)
\end{equation}
...
\begin{equation}
\zeta = \lbrace \sigma_x, A, U_{1..K}, \alpha_g, \zeta_g, \alpha_z, \zeta_z \rbrace
\end{equation}
%
...
= \log \mathcal{N}(x;zAv,\sigma_x I) + \log \mathcal{N}(v;0,C_v) + \log \textrm{Gam}(g;\alpha_g,\zeta_g) + \log \textrm{Gam}(z;\alpha_z,\zeta_z)
\end{split}
\end{equation}
%
...
+ \sum_{j=1}^K \left[ (\alpha_g - 1) \log(g_j) - \frac{g_j}{\zeta_g} \right] + (\alpha_z - 1) \log(z) - \frac{z}{\zeta_z}
\end{split}
\end{equation}
%
...
- \frac{1}{2} \left[ \log \left( \det \left( C_v \right) \right) + v^T C_v^{-1} v \right] + \\
+ \sum_{j=1}^K \left[ (\alpha_g - 1) \log(g_j) - \frac{g_j}{\zeta_g} \right] + \\
+ (\alpha_z - 1) \log(z) - \frac{z}{\zeta_z}
\end{split}
\end{equation}
...
+ \sum_{j=1}^K \left[ (\alpha_g - 1) \log(g_j) - \frac{g_j}{\zeta_g} \right]   + f_2(v,z)
\end{split}
\end{equation}
%
...
+ \frac{\partial}{\partial g_i} \left[ (\alpha_g - 1) \log(g_i) - \frac{g_i}{\zeta_g} \right]
\end{split}
\end{equation}
%
...
\begin{equation}
\frac{\partial}{\partial g_i} \log p(v,g,z \mid x) =  - \frac{1}{2} \textrm{Tr} \left[ \left[ C_v^{-1} - C_v^{-1} v v^T C_v^{-1} \right] C_i \right]  + \frac{\alpha_g - 1}{g_i} - \frac{1}{\zeta_g}
\end{equation}
...
\begin{equation}
\log p(v,g,z \mid x) \sim - \frac{z}{2 \sigma_x} \left( zv^TA^TAv - 2x^TAv \right) + (\alpha_z - 1) \log(z) - \frac{z}{\zeta_z} + f_3(g,v)
\end{equation}

\begin{equation}
\frac{\partial}{\partial z} \log p(v,g,z \mid x) =  \frac{1}{\sigma_x} \left[ x^TAv - z v^T A^TA v \right] + \frac{\alpha_z - 1}{z} - \frac{1}{\zeta_z}
\end{equation}
...
\begin{eqnarray}
\log p(g,z \mid x) \sim -\frac{1}{2} \left[ \log \det (C_x) + x^T C_x^{-1} x \right] + (\alpha_z-1)\log(z) - \frac{z}{\zeta_z} + (\alpha_g-1)\sum_{k=1}^K \log(g_k) - \frac{1}{\zeta_g} \sum_{k=1}^K g_k \\
